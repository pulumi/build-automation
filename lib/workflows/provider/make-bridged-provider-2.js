...
exports.bridgedProviderV2 = void 0;
function bridgedProviderV2(config) {
    var _a, _b;
    const PACK = config.provider;
    const ORG = "pulumi";
    const PROJECT = `github.com/$(ORG)/pulumi-$(PACK)`;
    const PROVIDER_PATH = config["major-version"] > 1
    ...
    const bin_pulumictl = {
        ...
        variables: {
            ...
            PULUMICTL_URL: "https://github.com/pulumi/pulumictl/releases/download/$(PULUMICTL_VERSION)/pulumictl-$(PULUMICTL_VERSION)-$(PLAT).tar.gz",
        },
        commands: [
    ...
    const make_install_plugins = {
        name: ".make/install_plugins",
        autoTouch: true,
        dependencies: [bin_pulumictl],
        commands: [
            "[ -x $(shell which pulumi) ] || curl -fsSL https://get.pulumi.com | sh",
            ...((_b = (_a = config.plugins) === null || _a === void 0 ? void 0 : _a.map((p) => `pulumi plugin install resource ${p.name} ${p.version}`)) !== null && _b !== void 0 ? _b : []),
        ],
    };
    ...
    const install_dotnet_sdk = {
        name: "install_dotnet_sdk",
        phony: true,
        dependencies: [make_install_dotnet_sdk],
    };
    const install_python_sdk = {
        name: "install_python_sdk",
        phony: true,
    };
    const install_java_sdk = {
        name: "install_java_sdk",
        phony: true,
    };
    const install_go_sdk = { name: "install_go_sdk", phony: true };
    const make_install_nodejs_sdk = {
        name: ".make/install_nodejs_sdk",
        autoTouch: true,
        commands: ["yarn link --cwd sdk/nodejs/bin"],
    };
    const install_nodejs_sdk = {
    ...
    const only_build = { name: "only_build", dependencies: [build] };
    const test = {
        name: "test",
        phony: true,
        commands: [
...
exports.bridgedProviderV2 = bridgedProviderV2;
//# sourceMappingURL=make-bridged-provider-2.js.map